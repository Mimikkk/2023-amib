...
class FramsticksLib:
  ...
  def satisfiesConstraints(self, genotype: str, max_numparts: int, max_numjoints: int, max_numneurons: int, max_numconnections: int,
                           max_numgenochars: int) -> bool:
    """
  ...
  def evaluate(self, genotype_list: List[str]):
    ...
    """
    assert isinstance(genotype_list,
                      list)  # because in python, str has similar capabilities as list and here it would pretend to work too, so to avoid any ambiguity
    ...
    for g in frams.GenePools[0]:
      ...
      evaluations = json.loads(
        serialized_dict._string())  # Framsticks native ExtValue's get converted to native python types such as int, float, list, str.
  ...
  def getRandomGenotype(self, initial_genotype: str, parts_min: int, parts_max: int, neurons_min: int, neurons_max: int, iter_max: int,
    ...
    for i in range(
        iter_max // 2):  # a sequence of iter_max/2 mutations, only accepting those which approach target numbers of parts and neurons
      g_new = self.mutate([g])[0]
  ...
  def isValidCreature(self, genotype_list: List[str]) -> List[bool]:
    ...
    pop = frams.Populations[
      0]  # assuming rules from population #0 (self-colision settings are population-dependent and can influence creature build success/failure)
    ...
    for g in genotype_list:
      if frams.Geno.newFromString(g).is_valid._int() != 1:
        ...
        can_add = pop.canAdd(g, 1,
                             1)  # First "1" means to treat warnings during build as build failures - this allows detecting problems when building Creature from Genotype. Second "1" means mute emitted errors, warnings, messages. Returns 1 (ok, could add) or 0 (there were some problems building Creature from Genotype)
