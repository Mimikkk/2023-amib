...
class ExtValue(object):
  ...
  _reservedWords = [
    'import']  # this list is scanned during every attribute access, only add what is really clashing with Framsticks properties
...
def init(*args):
  ...
  for a in args:
    if a[:2] == '-d':
      ...
      print(
        "frams.py: thread synchronization enabled.")  # Due to performance penalty, only use if you are really calling methods from different threads.
      ...
      for name in ExtValue.__dict__:
        ...
        if callable(
            attr) and attr:  # decorate all methods of ExtValue with a reentrant lock so that different threads do not use them concurrently
  ...
  if os.name == 'nt':
    if sys.version_info < (3, 8):
      ...
      os.chdir(
        lib_path)  # because under Windows, frams-objects.dll requires other dll's which reside in the same directory, so we must change current dir for them to be found while loading the main dll.
    else:
      os.add_dll_directory(os.path.abspath(lib_path))
  abs_data = os.path.join(os.path.abspath(lib_path),
                          "data")  # use absolute path for -d and -D so python is free to cd anywhere without confusing Framsticks
  ...
  if lib_path is not None:  # theoretically, this should only be needed for "and os.name == 'posix'", but in windows python 3.9.5, without using the full lib_name path, there is FileNotFoundError: Could not find module 'frams-objects.dll' (or one of its dependencies). Try using the full path with constructor syntax. Maybe related: https://bugs.python.org/issue42114 and https://stackoverflow.com/questions/59330863/cant-import-dll-module-in-python and https://bugs.python.org/issue39393
    lib_name = os.path.join(lib_path,
                            lib_name)  # lib_path is always set ('.' when not specified). For the (currently unused) case of lib_path==None, the resulting lib_name is a bare filename without any path, which loads the library from a system-configured location.
  try:
    c_api = ctypes.CDLL(
      lib_name)  # if accessing this module from multiple threads, they will all share a single c_api and access the same copy of the library and its data. If you want separate independent copies, read the comment at the top of this file on using the "multiprocessing" module.
  except OSError as e:
